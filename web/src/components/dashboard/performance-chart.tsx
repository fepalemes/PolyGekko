...
export function PerformanceChart({ samples, polyActivity = [], isDryRun = true }: PerformanceChartProps) {
  const { t } = useLang();
  ...
  // Build live chart data from polyActivity (cumulative net cashflow over time)
  const isLiveMode = !isDryRun && polyActivity.length > 0;

  if (isLiveMode) {
    const sorted = [...polyActivity].sort((a, b) => a.timestamp - b.timestamp);
    let cum = 0;
    const liveData = sorted.map(a => {
      const delta = (a.side === 'SELL' || a.type === 'REDEEM') ? a.usdcSize : -a.usdcSize;
      cum += delta;
      return {
        time: format(new Date(a.timestamp * 1000), 'MM/dd HH:mm'),
        LIVE: parseFloat(cum.toFixed(2)),
      };
    });

    const strategyTypes = ['LIVE'];
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="ml-2 text-xs text-muted-foreground">Polymarket</span>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={liveData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              <defs>
                <linearGradient id="grad-LIVE" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%"  stopColor={strategyColors.LIVE} stopOpacity={0.18} />
                  <stop offset="95%" stopColor={strategyColors.LIVE} stopOpacity={0} />
                </linearGradient>
              </defs>
              ...
              {!hidden.includes('LIVE') && (
                <Area
                  type="monotone"
                  dataKey="LIVE"
                  stroke={strategyColors.LIVE}
                  fill="url(#grad-LIVE)"
                  strokeWidth={2}
                  dot={false}
                  connectNulls
